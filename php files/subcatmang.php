...
if (!isset($_SESSION["pname"]) || $_SESSION["usertype"] !== "admin") {
    // header("location:admin_login.php"); // Or your specific admin login
    // exit;
}

$add_message = ""; // To store success/error messages for adding sub-category

if (isset($_POST["sbtn"])) {
    require_once("vars.php"); // For db credentials

    $cid = $_POST["cat"]; // Category ID
    $scatname = trim($_POST["subcatname"]); // Trim whitespace
    $subcatpic = "defaultpic.png"; // Default picture

    // Basic validation
    if (empty($cid) || $cid == "Choose Category") { // Ensure a category is selected
        $add_message = "<span class='text-danger'>Please select a parent category.</span>";
    } elseif (empty($scatname)) {
        $add_message = "<span class='text-danger'>Sub-Category Name cannot be empty.</span>";
    } else {
        if (isset($_FILES["subcatpic"]) && $_FILES["subcatpic"]["error"] == 0) {
            $allowed_types = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
            $file_type = $_FILES["subcatpic"]["type"];

            if (in_array($file_type, $allowed_types)) {
                $subcatpic_name_only = basename($_FILES["subcatpic"]["name"]);
                $subcatpic = time() . "_" . preg_replace("/[^a-zA-Z0-9.\-_]/", "_", $subcatpic_name_only);
                $tempname = $_FILES["subcatpic"]["tmp_name"];
                if (!move_uploaded_file($tempname, "uploads/$subcatpic")) {
                    $add_message = "<span class='text-danger'>Failed to upload sub-category picture. Using default.</span>";
                    $subcatpic = "defaultpic.png";
                }
            } else {
                $add_message = "<span class='text-warning'>Invalid file type for sub-category picture. Using default. Allowed: JPG, PNG, GIF, WEBP.</span>";
                $subcatpic = "defaultpic.png";
            }
        }
        ...
        $stmt = mysqli_prepare($connection, "INSERT INTO subcat (catid, subcatname, subcatpic) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "iss", $cid, $scatname, $subcatpic); // catid is likely an integer

        if (mysqli_stmt_execute($stmt)) {
            if (mysqli_stmt_affected_rows($stmt) == 1) {
                $add_message = "<span class='text-success'>Sub-Category Added Successfully.</span>";
            } else {
                $add_message = "<span class='text-danger'>Sub-Category Not Added. Possible duplicate or database issue.</span>";
            }
        } else {
            $add_message = "<span class='text-danger'>Error adding sub-category: " . htmlspecialchars(mysqli_stmt_error($stmt)) . "</span>";
        }
...
?>
    ...
    <head>
        ...
        <style>
            ...
            .form-section-wrapper .form-select, /* Added .form-select */
            .form-section-wrapper .form-control[type="file"] {
                background-color: var(--input-bg); color: var(--text-color);
                border: 1px solid var(--border-color); border-radius: 6px;
                padding: .5rem .75rem;
            }
            .form-section-wrapper .form-select option { /* Style for options in select */
            ...
            .form-section-wrapper .form-select:focus { /* Added .form-select */
            ...
            .table-subcategories { /* Renamed for clarity */
                width: 100%; margin-bottom: 0; border-collapse: separate;
                border-spacing: 0; font-size: 0.9rem;
            }
            .table-subcategories th, .table-subcategories td {
            ...
            .table-subcategories thead th:first-child { border-top-left-radius: calc(var(--card-border-radius) - 1px); }
            .table-subcategories thead th:last-child { border-top-right-radius: calc(var(--card-border-radius) - 1px); }
            .table-subcategories tbody tr {
                background-color: var(--table-row-bg);
                transition: background-color var(--transition-speed) ease;
            }
            .table-subcategories tbody tr:hover { background-color: var(--table-row-hover-bg); }
            .table-subcategories tbody tr:hover td { color: #fff; }
            .table-subcategories tbody tr:last-child td:first-child { border-bottom-left-radius: calc(var(--card-border-radius) - 1px); }
            .table-subcategories tbody tr:last-child td:last-child { border-bottom-right-radius: calc(var(--card-border-radius) - 1px); }
            .table-subcategories img {
                max-width: 60px; height: auto; border-radius: 4px;
                border: 1px solid var(--border-color);
                background-color: rgba(var(--primary-bg),0.5);
            }
            .table-subcategories .action-link {
                color: var(--accent-color); font-weight: 500; text-decoration: none;
                transition: color var(--transition-speed), text-shadow var(--transition-speed);
                padding: 5px 8px; border-radius: 4px;
            }
            .table-subcategories .action-link:hover {
                color: #fff; text-shadow: 0 0 5px var(--glow-color);
                background-color: rgba(var(--accent-color-rgb), 0.2);
            }
            .table-subcategories .action-link.delete { color: var(--danger-color); }
            .table-subcategories .action-link.delete:hover {
            ...
            @media (max-width: 768px) {
                ...
                .table-subcategories th, .table-subcategories td { padding: 0.7rem 0.5rem; font-size: 0.85rem; }
                .table-subcategories img { max-width: 45px; }
            }
        </style>
    </head>
    <body>
    ...
    <div class="container page-main-content px-md-4 px-2">
        <div class="text-center">
            <h1 class="page-title display-5">Manage Sub-Categories</h1>
        </div>

        <!-- Add Sub-Category Form Section -->
        <div class="form-section-wrapper">
            <h2 class="section-heading"><i class="fas fa-sitemap me-2"></i>Add New Sub-Category</h2>
                    ...
                    <div class="col-md-8 col-lg-6">
                        <div class="mb-3">
                            ...
                            <select name="cat" id="cat" class="form-select form-select-lg" required>
                                ...
                                <?php
                                require_once("vars.php"); // Ensure vars is available
                                $conn_cat_list = mysqli_connect(dbhost, dbuname, dbpass, dbname) or die("Error in connection" . mysqli_connect_error());
                                $q_cat = "SELECT catid, catname FROM managecat ORDER BY catname ASC";
                                $res_cat = mysqli_query($conn_cat_list, $q_cat) or die("Error in query" . mysqli_error($conn_cat_list));

                                if (mysqli_num_rows($res_cat) == 0) { // Use mysqli_num_rows
                                    echo "<option value='' disabled>No Categories Available</option>";
                                } else {
                                    while ($resarr_cat = mysqli_fetch_array($res_cat)) {
                                        echo "<option value='" . htmlspecialchars($resarr_cat[0]) . "'>" . htmlspecialchars($resarr_cat[1]) . "</option>";
                                    }
                                }
                        ...
                        <div class="mb-3">
                            <label for="subcatname" class="form-label visually-hidden">Sub-Category Name</label>
                            <input type="text" name="subcatname" id="subcatname" placeholder="New Sub-Category Name" class="form-control form-control-lg" required>
                        </div>
                        <div class="mb-3">
                            <label for="subcatpic" class="form-label visually-hidden">Sub-Category Picture</label>
                            <input type="file" name="subcatpic" id="subcatpic" class="form-control form-control-lg">
                        ...
                        <div class="text-center mt-4">
                            <button type="submit" name="sbtn" class="btn btn-submit btn-lg"><i class="fas fa-save me-2"></i>Add Sub-Category</button>
                        </div>
        ...
        <!-- List Sub-Categories Section -->
        <div class="table-section-wrapper mt-5">
            <h2 class="section-heading"><i class="fas fa-list-ul me-2"></i>Existing Sub-Categories</h2>
            <div class="table-responsive-custom">
                <?php
                // require_once("vars.php"); // Already included if needed earlier
                $connection_list = mysqli_connect(dbhost, dbuname, dbpass, dbname) or die("Error in connection" . mysqli_connect_error());
                // Join with managecat to display parent category name
                $q_list = "SELECT s.subcatid, s.catid, c.catname, s.subcatname, s.subcatpic 
                       FROM subcat s
                       JOIN managecat c ON s.catid = c.catid
                       ORDER BY c.catname ASC, s.subcatname ASC";
                $res_list = mysqli_query($connection_list, $q_list) or die("Error in query" . mysqli_error($connection_list));
                $rowcount_list = mysqli_num_rows($res_list);

                if ($rowcount_list == 0) {
                    echo "<div class='alert alert-info'>No sub-categories found. Add one using the form above.</div>";
                } else {
                    echo "<table class='table-subcategories'>"; // Renamed class
                    echo "<thead><tr><th>Sub ID</th><th>Parent Category</th><th>Sub-Category Name</th><th>Picture</th><th>Update</th><th>Delete</th></tr></thead><tbody>";
                    while ($resarray = mysqli_fetch_assoc($res_list)) { // Use fetch_assoc for named keys
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($resarray['subcatid']) . "</td>";
                        echo "<td>" . htmlspecialchars($resarray['catname']) . " (ID: " . htmlspecialchars($resarray['catid']) . ")</td>";
                        echo "<td>" . htmlspecialchars($resarray['subcatname']) . "</td>";
                        echo "<td><img src='uploads/" . htmlspecialchars($resarray['subcatpic']) . "' alt='" . htmlspecialchars($resarray['subcatname']) . "'></td>";
                        echo "<td><a href='updatesubcat.php?subcid=" . urlencode($resarray['subcatid']) . "' class='action-link update'><i class='fas fa-edit me-1'></i>Update</a></td>";
                        echo "<td><a href='delsubcat.php?subcatid=" . urlencode($resarray['subcatid']) . "' class='action-link delete' onclick=\"return confirm('Are you sure you want to delete sub-category: " . htmlspecialchars(addslashes($resarray['subcatname']), ENT_QUOTES) . "? This might affect products.');\"><i class='fas fa-trash-alt me-1'></i>Delete</a></td>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";
                }
